...
void TempHumRead(void) {
    ...
    if (scd4x.getHumidity() == 0) {  // Obtain the data of shT30.  获取sht30的数据
        temp = scd4x.getTemperature();  // Store the temperature obtained from shT30.
        // 将sht30获取到的温度存储
        humd = scd4x.getHumidity();  // Store the humidity obtained from the SHT30.
        // 将sht30获取到的湿度存储
    }
